//! Single-line text input, adapted from GPUI 0.2.2's `examples/input.rs`
//! (Apache-2.0, Zed Industries). GPUI ships no text-field widget, so this
//! is the upstream example plus: placeholder, change/submit events, an
//! optional numeric-only mode, and Anorak styling.
...
pub struct TextInput {
    ...
    /// Digits (and one '.') only, for the numeric filter fields.
    numeric: bool,
    height: Pixels,
    text_size: Pixels,
...
impl TextInput {
    ...
    fn sanitize(&self, new_text: &str) -> String {
        if !self.numeric {
            return new_text.replace('\n', " ");
        }
        let mut seen_dot = self.content.contains('.');
        new_text
            .chars()
            .filter(|c| {
                if c.is_ascii_digit() {
