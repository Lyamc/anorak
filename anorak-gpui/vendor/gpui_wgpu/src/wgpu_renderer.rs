...
/// Render pipelines, created on first use.
///
/// anorak patch: on the web each pipeline is a WebGL program whose link blocks
/// the main thread in `getProgramParameter` (ANGLE compiles the shaders there),
/// roughly 50 ms apiece on a cold shader cache. Creating all nine up front cost
/// about 400 ms before the first frame, although a simple UI draws only quads
/// and text. Pipelines are therefore built lazily on wasm. Native builds still
/// create them all in `create_pipelines`, as upstream does.
struct WgpuPipelines {
    builder: PipelineBuilder,
    quads: OnceCell<wgpu::RenderPipeline>,
    shadows: OnceCell<wgpu::RenderPipeline>,
    path_rasterization: OnceCell<wgpu::RenderPipeline>,
    paths: OnceCell<wgpu::RenderPipeline>,
    underlines: OnceCell<wgpu::RenderPipeline>,
    mono_sprites: OnceCell<wgpu::RenderPipeline>,
    subpixel_sprites: OnceCell<wgpu::RenderPipeline>,
    poly_sprites: OnceCell<wgpu::RenderPipeline>,
    #[allow(dead_code)]
    surfaces: OnceCell<wgpu::RenderPipeline>,
}
...
/// Everything needed to build any of the pipelines later.
struct PipelineBuilder {
    device: wgpu::Device,
    globals_layout: wgpu::BindGroupLayout,
    instances_layout: wgpu::BindGroupLayout,
    texture_layout: wgpu::BindGroupLayout,
    surfaces_layout: wgpu::BindGroupLayout,
    shader_module: wgpu::ShaderModule,
    subpixel_shader_module: Option<wgpu::ShaderModule>,
...
impl PipelineBuilder {
    ...
    fn create_pipeline(
        ...
        let mut bind_group_layouts = vec![Some(&self.globals_layout), Some(data_layout)];
    ...
    fn build(&self, kind: PipelineKind) -> wgpu::RenderPipeline {
        let strip = wgpu::PrimitiveTopology::TriangleStrip;
        let color = || [Some(self.color_target.clone())];
        let instances = &self.instances_layout;
        let texture = Some(&self.texture_layout);
        let module = &self.shader_module;
        match kind {
            PipelineKind::Quads => {
                self.create_pipeline("quads", "vs_quad", "fs_quad", instances, None, strip, &color(), 1, module)
            }
            PipelineKind::Shadows => self.create_pipeline(
                "shadows", "vs_shadow", "fs_shadow", instances, None, strip, &color(), 1, module,
            ),
            PipelineKind::PathRasterization => self.create_pipeline(
                "path_rasterization",
            ...
            PipelineKind::Underlines => self.create_pipeline(
                "underlines", "vs_underline", "fs_underline", instances, None, strip, &color(), 1, module,
            ),
            PipelineKind::MonoSprites => self.create_pipeline(
                "mono_sprites", "vs_mono_sprite", "fs_mono_sprite", instances, texture, strip, &color(), 1, module,
            ),
            PipelineKind::SubpixelSprites => {
                ...
                self.create_pipeline(
                    ...
                    1,
                    self.subpixel_shader_module
                        .as_ref()
                        .expect("subpixel pipeline requested without dual-source blending"),
                )
            }
            PipelineKind::PolySprites => self.create_pipeline(
                "poly_sprites", "vs_poly_sprite", "fs_poly_sprite", instances, texture, strip, &color(), 1, module,
            ),
            PipelineKind::Surfaces => self.create_pipeline(
                "surfaces", "vs_surface", "fs_surface", &self.surfaces_layout, None, strip, &color(), 1, module,
            ),
        }
    }
}
...
impl WgpuPipelines {
    fn get<'a>(&'a self, cell: &'a OnceCell<wgpu::RenderPipeline>, kind: PipelineKind) -> &'a wgpu::RenderPipeline {
        cell.get_or_init(|| {
            log::debug!("creating {kind:?} pipeline");
            self.builder.build(kind)
        })
    }
    fn quads(&self) -> &wgpu::RenderPipeline {
        self.get(&self.quads, PipelineKind::Quads)
    }
    fn shadows(&self) -> &wgpu::RenderPipeline {
        self.get(&self.shadows, PipelineKind::Shadows)
    }
    fn path_rasterization(&self) -> &wgpu::RenderPipeline {
        self.get(&self.path_rasterization, PipelineKind::PathRasterization)
    }
    fn paths(&self) -> &wgpu::RenderPipeline {
        self.get(&self.paths, PipelineKind::Paths)
    }
    fn underlines(&self) -> &wgpu::RenderPipeline {
        self.get(&self.underlines, PipelineKind::Underlines)
    }
    fn mono_sprites(&self) -> &wgpu::RenderPipeline {
        self.get(&self.mono_sprites, PipelineKind::MonoSprites)
    }
    fn subpixel_sprites(&self) -> Option<&wgpu::RenderPipeline> {
        self.builder.subpixel_shader_module.as_ref()?;
        Some(self.get(&self.subpixel_sprites, PipelineKind::SubpixelSprites))
    }
    fn poly_sprites(&self) -> &wgpu::RenderPipeline {
        self.get(&self.poly_sprites, PipelineKind::PolySprites)
    }
    #[allow(dead_code)]
    fn surfaces(&self) -> &wgpu::RenderPipeline {
        self.get(&self.surfaces, PipelineKind::Surfaces)
    }
}
...
impl WgpuRendererCore {
    ...
    fn create_pipelines(
        ...
        let pipelines = WgpuPipelines {
            builder: PipelineBuilder {
                device: device.clone(),
                globals_layout: layouts.globals.clone(),
                instances_layout: layouts.instances.clone(),
                texture_layout: layouts.texture.clone(),
                surfaces_layout: layouts.surfaces.clone(),
                shader_module,
                subpixel_shader_module,
                surface_format,
                color_target,
                path_sample_count,
            },
            quads: OnceCell::new(),
            shadows: OnceCell::new(),
            path_rasterization: OnceCell::new(),
            paths: OnceCell::new(),
            underlines: OnceCell::new(),
            mono_sprites: OnceCell::new(),
            subpixel_sprites: OnceCell::new(),
            poly_sprites: OnceCell::new(),
            surfaces: OnceCell::new(),
        };
        // Native: build everything now, as upstream does. Web: on first use.
        #[cfg(not(target_family = "wasm"))]
        {
            pipelines.quads();
            pipelines.shadows();
            pipelines.path_rasterization();
            pipelines.paths();
...
impl WgpuRenderer {
    ...
    pub fn update_drawable_size(&mut self, size: Size<DevicePixels>) {
        ...
        // anorak patch: not on wasm. Browsers keep destroyed textures alive until
        // queued work finishes, WebGPU's poll is a no-op, and WebGL cannot block
        // (wgpu waits with a zero timeout), so this only logged a misleading
        // "Failed to poll device during resize: Timeout" warning.
        #[cfg(not(target_family = "wasm"))]
                    ...
                    PrimitiveBatch::SubpixelSprites { texture_id, range } => {
                        ...
                        self.draw_sprites(
                            ...
                            resources
                                .pipelines
                                .subpixel_sprites()
                                .unwrap_or_else(|| resources.pipelines.mono_sprites()),
                            instance_range(range),
                            &mut pass,
                        )?;
                    }
